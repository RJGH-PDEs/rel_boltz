"""
5-panel figure: one panel per m_i value of the l_i=2 test function.

Each panel shows the normalized growth |Q_{k_i,l_i=2,m_i,s,t}| / |Q_{0,l_i=2,m_i,s,t}|
on a log scale for k_i=0,1,2, with one curve per nonzero (s,t) entry, coloured by
(l_s,l_t).

Run from plot/:
    ~/miniconda3/envs/ttenv/bin/python plot_ki_growth_l2.py
Writes: ../plot/figures/ki_growth_l2.png
"""
...
# colour by (l_s, l_t) — discover unique pairs across all panels
all_pairs = set()
for m_i in M_VALS:
...
# ── figure: 1 row × 5 panels ──────────────────────────────────────────────────
fig, axes = plt.subplots(1, 5, figsize=(16, 4.5), sharey=True)
fig.suptitle(
    r'Normalized $k_i$-growth for test functions $(\ell_i=2,\,m_i)$, all nonzero $(s,t)$'
    '\n' r'$|Q_{k_i}|\,/\,|Q_0|$,  log scale  [n=3]',
    fontsize=11
)
...
for ax, m_i in zip(axes, M_VALS):
    ...
    for f, g in zip(rows.tolist(), cols.tolist()):
        v0 = abs(float(mat0[f, g]))
        if v0 < 1e-20:
            continue
        seq = np.array([abs(float(mats[ki][f, g])) / v0 for ki in range(3)])
        _, ls, _ = ind_to_klm(f, n)
        _, lt, _ = ind_to_klm(g, n)
        ax.plot(KI_VALS, seq,
...
handles = [
    mlines.Line2D([], [], color=pair_color[p], lw=2, marker='o', ms=5,
                  label=rf'$l_s={p[0]},\,l_t={p[1]}$')
    for p in unique_pairs
]
fig.legend(handles=handles, title=r'$(l_s,\,l_t)$',
           loc='lower center', ncol=len(unique_pairs),
           fontsize=9, bbox_to_anchor=(0.5, -0.05))

plt.tight_layout(rect=[0, 0.1, 1, 1])
outpath = 'figures/ki_growth_l2.png'
